defstar {
	...
	domain { MDSDF }
	desc {
Quantize the input value to one of N+1 possible output levels using N
thresholds.  For an input less than or equal to the n-th threshold, but
larger than all previous thresholds, the output will be the n-th level.
If the input is greater than all thresholds, the output is the N+1-th
level.  If level is specified, there must be one more level than
thresholds; the default value for level is 0, 1, 2, ... N.  This star
is much slower than LinQuantIdx, so if possible, that one should be
used instead.
	}
	version { @(#)MDSDFQuant.pl	1.4	03/09/96 }
...
limitation of liability, and disclaimer of warranty provisions.
	...
	go {
		...
		int numentries = int(numRows)*int(numCols);
