...
is at a Fork input, it is \code{F\_SRC}. If it is at a Fork output, it is
\code{F\_DEST}.
...
We do not allocate a buffer to a CGGeodesic if it is \code{F\_DEST}.
...
yet been run. If this Geodesic is a \code{F\_SRC}, the minimum size
...
an environment variable \code{PT\_DISPLAY}. Two examples are \code{vi} and
