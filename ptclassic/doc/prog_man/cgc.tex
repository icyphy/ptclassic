...
\code{P_GLOBAL} and \code{P_LOCAL} are defined for CGC portholes.
There are several special code-generating methods:  \code{addInclude},
...
\code{CGCStar}\tindex{CGCStar class} class.
...
\method{int}{addDeclaration}
{(const char* \var{text}, const char* \var{name} = NULL)}
Conditionally add \var{text} to the
\code{mainDecls}\vindex{mainDecls!CGCTarget member} stream.  Use
\var{name} as the identifying key for the code fragment if it is
provided, otherwise use \var{text} itself as the key.

\method{int}{addGlobal}
{(const char* \var{text}, const char* \var{name} = NULL)}
Conditionally add \var{text} to the
\code{globalDecls}\vindex{globalDecls!CGCTarget method} stream.  Use
\var{name} as the identifying key for the code fragment if it is
provided, otherwise use \var{text} itself as the key.

\method{int}{addMainInit}
{(const char* \var{text}, const char* \var{name} = NULL)}
...
this time.  Buffer embedding is used by \code{Spread}\tindex{Spread!CGC
block}, \code{Collect}\tindex{Collect!CGC block},
\code{Switch}\tindex{Switch!CGC block}, and
\code{Select}\tindex{Select!CGC block}.
...
must use the \code{A_GLOBAL}\vindex{A_GLOBAL attribute}
(\code{P_GLOBAL}\vindex{P_GLOBAL attribute}) attribute.

The \code{CGCTclTkTarget} class defines the
\code{tkSetup}\vindex{tkSetup!CGCTclTkTarget stream for Tcl/Tk stars.
member}There is no special code generation function for this stream, so
its name must be used with \code{addCode}\findex{addCode!CGStar method}.
...
This functions creates a popup window containing \var{message}.
...
\code{addGlobal()}\findex{addGlobal!CGCStar method}
...
\code{addDeclaration}\findex{addDeclaration, method}
and 
\code{addInclude()}\findex{addInclude method}
...
The default target \code{default-CGC}\tindex{default-CGC target}, which
is implemented by the \code{CGCTarget}\tpindexbold{CGCTarget class}
...
\code{CGCTarget} class listed in table\tie\ref{CGCTarget-parameters}.
...
Code generated by the \code{wrapup}\findex{wrapup!Star method} method of
...
\code{addMainInit}\findex{addMainInit!CGCStar method} method of
\code{CGCStar}.
...
The following streams are defined as members of the
\code{CGCTclTkTarget}\tindex{CGCTclTkTarget class} class in addition to the
streams defined by the \code{CGCTarget}\tindex{CGCTarget class} class.

\begin{indexlist}{CGCTclTkTarget}
\member{CodeStream}{mainLoopInit}
This stream is used internally by the target class.
\member{CodeStream}{mainLoopTerm}
This stream is used internally by the target class.
\member{CodeStream}{tkSetup}
