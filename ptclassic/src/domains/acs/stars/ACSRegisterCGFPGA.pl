defcore {
	...
	version {@(#)ACSRegisterCGFPGA.pl	1.0 09/10/99}
	author { K. Smith }
	copyright {
...
This star exists only for demoing the generic CG domain.
	...
	defstate {
	    name {offset}
	    type {int}
	    desc {Index of sample to repeat}
	    default {0}
	    attributes {A_NONCONSTANT|A_SETTABLE}
	}
	defstate {
	...
	method {
	    ...
	    code {
		// BEGIN-USER CODE
		int numTimes = intparam_query("numTimes");
		...
		cost_file << "cost=ceil(0.5*insizes);" << endl;
                ...
                numsim_file <<  "   y{k}=x{k}(1+" << offset << ")*ones(" << numTimes << ",1);" << endl;
		...
		natcon_file << "yesno=ones(1,size(insizes,2));" << endl;
                ...
                schedule_file << " racts1=[0 1 vl1-1; " << offset << "+1 1 " << numTimes+offset << "];" << endl;
                schedule_file << " racts=cell(1,size(insizes,2));" << endl;
                schedule_file << " racts(:)=deal({racts1});" << endl;
                schedule_file << " minlr=max(vl1," << numTimes << ")*ones(1,size(insizes,2)); " << endl;
		schedule_file << " if (vl1 < " << offset << "+1)" << endl;
		schedule_file << " disp('Warning: in ACSRegisterCGFPGA.pl, offset larger than input vector length')" << endl;
...
//		printf("UnitDelay delay_count=%d\n",delay_count);
	...
	method {
		...
		pins->set_phasedependency(3,OUTPUT_PIN);
