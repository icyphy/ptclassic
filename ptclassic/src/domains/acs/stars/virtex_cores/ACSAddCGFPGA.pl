...
It outputs lines of comments, instead of code.
	...
	method {
		...
		// BEGIN-USER CODE
		{
		    ...
		    // Correct Output S
		    printf("adder %s, has S_bitlen=%d, adder_length=%d\n",
			   name(),
			   S_bitlen,
			   adder_length);
			...
			// Core generator info
			out_core << "SET BusFormat = BusFormatAngleParen" << endl;
			...
			out_core << "CSET input_width = " << pins->query_bitlen(0) << endl;
