...
//		printf("UnitDelay delay_count=%d\n",delay_count);
	...
	method {
		...
		else if (fpga_type()==VIRTEX)
		{
		    ...
		    // Control port definitions
		    pins->add_pin("G",INPUT_PIN_CLK);
		    pins->add_pin("GE",INPUT_PIN_CE,AH);
		    pins->add_pin("SCLR",INPUT_PIN_CLR,AL);
		}
		...
		// BEGIN-USER CODE
		{
			...
			// Core generator info
			out_fstr << "SET BusFormat = BusFormatAngleParen" << endl;
		    ...
		    {
			// Library info
			corelib_fstr << "LD-based_Parallel_Latch|xilinx|virtex_all|1.0=active" << endl;
			...
			if (execlp("coregen",
				   ...
				   ".",
				   (char*) 0)==-1)
			    printf("ACSUnitDelayCGFPGA:Error, Unable to spawn a coregen session\n");
