...
main (int argc, char *argv[])
{
  ...
  vis_s16  *dst0,*src,*srcptolemy,*filtertaps,*shift_taparray,*indexcount;
...
dst1 = (vis_d64 *) memalign(sizeof(double),sizeof(double)*LENGTH/4);
src = (vis_s16 *) memalign(sizeof(double),sizeof(short)*LENGTH);
...
/***************************************************************/
  for(i=0;i<LENGTH;i++){
    ...
    src[i] = (short)SCALE * (double) sin(sinarg);
  }
...
/***************************************************************/
  start = gethrtime();
  for (j = 0; j < ntime; j ++) {
    vdk_c_ptolemyfir(src, dst0, length, filtertaps, taplength);
  }
  end = gethrtime();
...
/***************************************************************/
  ...
  for(i=0;i<length/4;i++){
   srcptolemy[NPACK*i+3]=src[i*NPACK];
   srcptolemy[NPACK*i+2]=src[i*NPACK+1];
   srcptolemy[NPACK*i+1]=src[i*NPACK+2];
   srcptolemy[NPACK*i]=src[i*NPACK+3];
  }
  ...
  start = gethrtime();
  for (j = 0; j < ntime; j ++) {
    vdk_vis_ptolemyfir((double*)srcptolemy, dst1, length/4,
