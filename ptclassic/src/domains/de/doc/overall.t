...
are also random.  This scheme, however, has problems.
...
defined as the negative of the longest path from the block 
...
correspond to the output ports of the wormhole.
The third kind is the \fIdelay-star\fRs which will be discussed
in the next section.  By defining the \fIdepth\fR of a terminating star to
be $-1$,
the \fIdepth\fR of any non-terminating star is less than $-1$.
.pp
The scheduler strategy is then simple.
If a star
has a smallest \fIdepth\fR among all stars with simultaneous input
events, the star will be scheduled first.
Therefore, when we put an event into the event queue, we first check the
time stamp of the event, and as a secondary sorting key,
compare the \fIdepth\fRs of the
...
of the system, a delay-free loop in the DE domain may be
intentional.  A delay-free loop in the DE domain is detected when
the \fIdepth\fRs of two stars
depend on each other.
In other words, if there is a loop of stars
with no terminating star,
the loop is called delay-free.
.pp
The difficulty comes from the
fact that the definition of the \fIdelay-star\fR is somewhat
ambiguous.  We define a DE star
as a \fIdelay-star\fR if it always outputs events
with time stamps strictly greater than the time stamps
of its input events.
But, there are some DE stars
which generate some outputs without delay
with respect to some inputs, and some outputs with a non-zero delay with
respect to some inputs.
A good example is a 
.c Queue
star.
...
.c Queue
star is not a delay-star.  But it is possible to make a delay-free
loop with it
without causing deadlock (or, for that matter, an infinite loop of events).
.pp
Although delay free loops are used, they are relatively uncommon,
and must be used with care.  If there is genuinely no delay in the loop,
for any input pattern, then the system is faulty.
Therefore, we generate a warning message if we detect a delay-free
...
.(f
\** see the document "\fBDomains in Ptolemy\fR".
.)f
...
in the \*(DO domain.  A strict definition is give above.
...
event comes.  The following example will help in understanding.
...
a \fIdelay\fR.
The last two lines will be explained below.
.pp
...
defstar {
	name {Server}
	domain {DE}
	desc { This star emulates a server.  }
	explanation {
...
etc).
.pp
A self-scheduling star is derived from the class
.cDERepeatStar
...
It processes the input event and generates an output event
without any latency.  If latency is desired in a \fIfunctional-star\fR, it can 
