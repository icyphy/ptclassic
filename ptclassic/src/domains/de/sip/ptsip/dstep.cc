...
{	SeqList *h; Element *ee;
	...
	{	ee = huntstart(h->thisseq->frst);
		filterbad(ee);
		switch (ee->n->ntyp) {
		case NON_ATOMIC:
			inh += CollectGuards(fd, ee->n->sl->thisseq->frst, inh);
			break;
		case  IF:
...
{	int isg=0; char buf[64];
	...
	case UNLESS:
		...
		return putcode(fd, s->frst->n->sl->thisseq, nxt, 0, ln);
	case NON_ATOMIC:
		(void) putcode(fd, s->frst->n->sl->thisseq, ZE, 1, ln);
		break;
	case IF:
		fprintf(fd, "if (!(");
	...
	fprintf(fd, " && !defined(BITSTATE) && !defined(MA)\n");
...
{	Element *e, *N;
	...
	{	if (e->status & DONE2)
		...
		{	filterbad(e);
			...
			case NON_ATOMIC:
				h = e->n->sl;
				putCode(fd, h->thisseq->frst,
					h->thisseq->extent, e->nxt, 0);
				break;
			case BREAK:
				if (LastGoto) break;
