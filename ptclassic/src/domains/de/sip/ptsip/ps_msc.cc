...
/* Copyright (c) 1997 by Lucent Technologies - Bell Laboratories.         */
...
int	pspno = 0;	/* postscript page */
int	ldepth = 1;
int	maxx;
int	TotSteps = 4096; /* max nr of steps, about 40 pages */
float	Scaler = (float) 1.0;
...
/*	extern int free(void *); */
	...
	{	if (!M[i] && !L[i]) continue;	/* no box here */
		...
		if (L[i]) { spitbox(M[i], 0, i, L[i]); tfree(L[i]); }
	}
	fprintf(pfd, "showpage\nrestore\n");
}
