...
This is implemented by replacing the SrcGrInt_M star with the named
block at setup time.
The replacement block(s) are connected as illustrated by the example
block that is connected.  Their parameters are determined by "parameter_map".
	}
	version { $Id$ }
	author { Edward A. Lee }
	copyright {
...
and output datatype to integer matrix.
...
.Sr "MapGr"
	...
	constructor {
	  ...
	  exin.setPort("exin", this, INT_MATRIX_ENV);
