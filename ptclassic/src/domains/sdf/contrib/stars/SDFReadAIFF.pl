...
limitation of liability, and disclaimer of warranty provisions.
   ...
   // Read the form chunk.
   Chunk FormChunk;
   if(!ReadChunk(FormChunk))
   {
      Error::abortRun(*this, "File is not a valid AIFF file");
      return;
   }

   // Ensure this is actually a FORM chunk.
   if (strncmp(FormChunk.ckID, "FORM", 4) != 0)
   {
      Error::abortRun(*this, "File is not a valid AIFF file");
      return;
   }
   ...
   while (ReadChunkHeader(Header))
   {
	 ...
	 if(!input)
	 {
	    Error::abortRun(*this, "Error reading AIFF file");
	    return;
	 }
	 ...
	 if(isAIFC) 
	 {
	    ...
	    ReadID(compType);
	    if(strncmp(compType, "NONE", 4))
	    {
	       Error::abortRun(*this, "Compressed AIFF file not supported.");
	       return;
	    }
	 ...
	 {
	    Error::abortRun(*this, "AIFF file format not recognized");
	    return;
	 }
      }
   ...
   if(sound_position < 0)
   {
      Error::abortRun(*this, "Unable to find sound data in AIFF file.");
      return;
   }
