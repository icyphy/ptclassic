...
.Se Burg
(AR) parameters of the input process.
 These parameters are then loaded
into the
.c BlockFIR
...
.c Burg
star must be equal.
Together they determine how many samples are processed by each iteration
of the overall schedule.
In this demo, they are both 256, so 256 input samples are processed on
each iteration.
...
Inputs are examined one-by-one and the current FIR filter coefficients
are used to predict the next sample.
...
the next prediction.
The plots show that that the prediction is not as good as that of the
Burg method, and that the initial transient is considerably longer,
giving time for the LMS filter to converge.
However, the adaptive filter method may perform better on some non-stationary
signals, such as those with slowly varying statistics.
