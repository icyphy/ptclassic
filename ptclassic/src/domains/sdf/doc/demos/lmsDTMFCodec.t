...
.NA lmsDTMFCodec
.SD
Dual-Tone Modulated-Frequency Coder-Decoder Using LMS
.EQ
delim off
.EN
...
.EN
.LD
.IE "direct frequency estimation technique"
.IE "LMS filter"
...
two LMS filters and decision logic to interpret the
output of the LMS filters [2].
The two LMS filters are three-tap filters in which the first and third
taps are fixed and the second tap is adapted.
The two filters attempt to lock onto the two strongest frequency components
in the input signal.
