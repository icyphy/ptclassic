...
<p>
The <i>ScriptDirectory</i></b> state specifies where to find customized Matlab
scripts and establishes a default directory for writing output files.
If <i>ScriptDirectory</i></b> is not empty, then the star will tell Matlab
...
it is best to specify a path name for the <i>ScriptDirectory</i></b>
...
<p>
The <i>DeleteOldFigures</i></b> setting allows the user to control whether
