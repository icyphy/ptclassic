...
The \dfn{CGC domain} is used to generate \code{C} code.  The chapter on
...
in \nxref{Writing code generation stars}, the attributes
\code{P_GLOBAL} and \code{P_LOCAL} are defined for CGC portholes.  There
are several special code-generating methods:  \code{addInclude},
\code{addDeclaration}, \code{addGlobal}, and \code{addMainInit}.  There
are also methods for buffer allocation:  \code{embed}.  All of these
...
with the \code{CGCPortHole} class. \tindex{CGCPortHole class}  They are
the counterparts of the \code{A_GLOBAL} \vindex{A_GLOBAL attribute}
and \code{A_LOCAL} \vindex{A_LOCAL attribute} attributes
...
addition to the \code{addCode} method described in chapter\tie\ref{CG
Domain}.  As with \code{addCode}, these methods return \samp{TRUE} if
code was generated for the appropriate stream and \samp{FALSE}
otherwise.  These methods are member functions of the \code{CGCStar}
\tindex{CGCStar class} class.

\begin{indexlist}{:CGCStar method}

\function{int}{addInclude}{(const char* \var{file})}
Generate the directive \samp{#include \var{file}} in the \code{include}
stream. \vindex{include:CGCTarget member} The string \var{file} must
include quotation marks ( ``name'' ) or angle brackets ( <name> )
around the name of the file to be included.  Only one \samp{#include
\var{file}} directive will be generated for the file, even if
\code{addInclude} is invoked multiple times with the same argument.
Return \samp{TRUE} if a new directive was generated.

\function{int}{addDeclaration}
{(const char* \var{text}, const char* \var{name} = NULL)}
Conditionally add \var{text} to the \code{mainDecls} stream.
\vindex{mainDecls:CGCTarget member} Use \var{name} as the identifying
key for the code fragment if it is provided, otherwise use \var{text}
itself as the key.

\function{int}{addGlobal}
{(const char* \var{text}, const char* \var{name} = NULL)}
Conditionally add \var{text} to the \code{globalDecls} stream.
\vindex{globalDecls:CGCTarget method} Use \var{name} as the identifying
key for the code fragment if it is provided, otherwise use \var{text}
itself as the key.

\function{int}{addMainInit}
{(const char* \var{text}, const char* \var{name} = NULL)}
Add \var{text} to the \code{mainInit} stream.
\vindex{mainInit:CGCTarget member} If \var{name} is not \samp{NULL},
...
this time.  Buffer embedding is used by \code{Spread}, \code{Collect},
\code{Switch}, and \code{Select}.
\tindex{Spread:CGC block}
\tindex{Collect:CGC block}
\tindex{Switch:CGC block}
\tindex{Select:CGC block}

\begin{indexlist}{:CGPortHole method}
\function{void}{embed}{(CGPortHole& \var{port}, int \var{location} = -1)}
...
buffers, otherwise return \sampe{FALSE}.

\function{int}{whereEmbedded}{()}
Return the offset of this porthole's buffer with respect to the buffer in which
it is embedded.

\function{void}{embedHere}{(int \var{location})}
...
\code{addGlobal()}
\cindex{addGlobal:CGCStar method}
...
\code{CGCTarget} \cpindexbold{CGCTarget, class} class, is a single
processor target.  The parameters described in the user's manual are
implemented by the state members of the \code{CGCTarget} class listed
...
The following streams are defined as members of the \code{CGCTarget}
\tindex{CGCTarget class} class in addition to the streams defined by
the \code{CGTarget} \tindex{CGTarget class} class.

\begin{indexlist}{:CGCTarget member}

\variable{CodeStream}{commInit}
...
\variable{CodeStream}{globalDecls} Global declarations for variables
and functions are added to this stream by the \code{addGlobal}
\findex{CGCStar::addGlobal} method of \code{CGCStar}.

\variable{CodeStream}{include} Include directives are added to this
stream by the \code{addInclude} \findex{CGCStar::addInclude} method of
\code{CGCStar}.

\variable{CodeStream}{mainClose}
Code generated by the \code{wrapup} \findex{wrapup method} method of
stars is placed in this stream.

\variable{CodeStream}{mainDecls}
Local declarations for variables are added to this stream by the
\code{addDeclaration} \findex{CGCStar::addDeclaration} method of
\code{CGCStar}.

\variable{CodeStream}{mainInit}
Initialization code is added to this stream by the \code{addMainInit}
\findex{CGCStar::addMainInit} method of \code{CGCStar}.

\variable{CodeStream}{wormIn}
This stream is used by the \code{wormInputCode} method.
\findex{wormInputCode:CGCTarget method}

\variable{CodeStream}{wormOut}
This stream is used by the \code{wormOutputCode} method.
\findex{wormOutputCode:CGCTarget method}
