...
//***********************************************************************
	...
	output { name { retranmitter }
                 type { message }
	         desc { retsart the timeout}
	}
	output { name { timeout}
                 type { int}
...
//***********************************************************************
	...
	protected { long count;
		    ...
		    float cwnd,ssthresh,Win,tmp_cwnd;
...
//***************************************************************************/
		  ...
		  if((Ack<Last_Ack)||(Time_Out&&(ret>2)&&(Ack==last_retransmitted)))
	           {
		      return;
		   }
		  if(Ack>Last_Ack)
...
OUTPUT<<" &&&&ACK "<<Seq<<" cwnd "<<cwnd<<" WIN "<<Win<<" RETRANSMISSION "<<Retransmission<<" at "<<completionTime<<"\n";
	          if(Ack!=Last_Ack)
	           ret=0;
// fast recovery //
		  if((ret>2)&&(Ack==last_retransmitted)&&!Time_Out)
		  {
OUTPUT<<" Limit "<<Limit<<" tmp_cwnd "<<tmp_cwnd<<" Last_Sent "<<Last_Sent<<"\n";
			tmp_cwnd+=MSS;
			lImit=tmp_cwnd+Ack;
                        demand.put(completionTime)<<Limit;//inform the buffer
                                                    //of the possibility to send
			return;
...
OUTPUT<<" (Seq+1)%Win_Size= "<<(Seq+1)%Win_Size<<" RTO "<<completionTime+RTO<<"\n";
		        ...
		        demand.put(completionTime)<<STOP;
		        Last_Seq=Seq+1;

			Slow_Start=0;  //do not go into slow start
			...
			if(cwnd<Win)   //reset the ssthresh value
			   ...
			   ssthresh-=(long)ssthresh%MSS;
		        ...
		        if(ssthresh<MSS) ssthresh=MSS+MSS; // set the lower
		                                           // limit of ssthresh
			tmp_cwnd=ssthresh+((float)3*(float)MSS);//init cwnd
			cwnd=ssthresh;
		        return;
		      }
	          }
		  ...
		  {
		    state=STOP;
		    demand.put(completionTime)<<state;
		  }
...
OUTPUT<<" Ack "<<Ack<<" Last_Sent "<<Last_Sent<<"\n";
		     if(!Time_Out)
		     {
			OUTPUT<<" Start Timer "<<Seq+1<<" RTO "<<RTO<<"\n";	
				int tmp=Seq+1;
				retranmitter.put(completionTime)<<buffer[(int)(tmp)%(int)Win_Size];
	                        timer_buf[(int)(tmp)%(int)Win_Size]=completionTime+RTO;
	                   	timeout.put(completionTime)<<RTO;
		     }
		     else
		     {
...
//****************************************************************************
			...
			Last_Sent=Sent_last;
if(debug)
OUTPUT<<" TIMEOUT "<<seq<<" send "<<last_seq+1<<" NUM "<<num<<" at "<<completionTime<<"\n";
//****************************************************************************
//clear all the timeout stamps in order to avoid timeout handling for packets
//that were transmitted but can not be acknowledged until a retransmitted
//packet reaches the receiver
//****************************************************************************
		     ...
		     for(i=0;i<Win_Size;i++)
	             ...
	             Retransmission=4;
		     Last_Seq=seq+1;
		     last_retransmitted=((float)Retransmitted+1)*(float)MSS;
		     Timer=-2;         //stop the RTO calculation
		     send_state=-1;
		     Limit=0;
			...
			cwnd=MSS;                      // initialize cwnd
		     ...
		     timeout.put(completionTime)<<RTO;// update the time out
	             ...
	             timer_buf[last_seq+1%Win_Size]=completionTime+RTO;
