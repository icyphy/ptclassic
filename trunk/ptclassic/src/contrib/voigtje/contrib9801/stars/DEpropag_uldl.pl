defstar {
	...
	author { A. Wernicke }
	version { @(#)DEpropag_uldl.pl	1.5 06/25/97}
	copyright { copyright (c) 1996 - 1997 Dresden University of Technology,
                    Mobile Communications Systems 
        }
	location { The WiNeS-Library }
        desc { This star implements a path loss model. A specific enviroment is
...
method {
    name { entfernen }
    access { private }
    arglist { "(Punkt& P1, Punkt& P2, int& I1, int& I2, double& Z1, double& Z2)" } 
    type { void }
    code {
...
method {
    name { Tuer_x_schliessen }
    access { private }
    arglist { "(double Tuerparam1, double Tuerparam2)" } 
    type { double }
    code {
...
method {
    name { Namenserfassung }
    access { private }
    arglist { "(char B_Name[14])" } 
    type { int }
    code {
...
method {                                                 // um double Zahlen hinreichend genau              
    name { Runden }                                  // miteinander vergleichen zu koennen                      
    access { private }
    arglist { "(double nmbr)" } 
    type { void }
    code {
