...
#define NUMPACK (4)
	...
	codeblock(mainDecl) {
	  ...
	  vis_d64  $starSymbol(betabott),$starSymbol(betatop),$starSymbol(filtertaps)[5];
	...
	codeblock(initialize) {
	  ...
	  settaps($starSymbol(filtertaps),$starSymbol(tapmatrix),&$starSymbol(betatop),&$starSymbol(betabott),$starSymbol(scaledown));
