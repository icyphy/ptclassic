...
unexpected jagged edges in the waveform.
 Reuse indicates that a sample appeared twice and means
that the next sample did not arrive in time, so the receiver
simply re-used the sample it had.
.pp
...
.c timeScale
is 1.0). 
 In this demo, we set the state at 2.0, which means one time unit
