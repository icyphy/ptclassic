...
static void	set_lab(Symbol *, Element *);
...
{	Element *lb, *f;
	...
	{	if (f && f->n && f->n->ntyp == GOTO)
		...
		{	if (!Rjumpslocal(h->thisseq->frst, h->thisseq->last))
				return 0;
...
{	int	tok = n->ntyp;
	...
	{	if (!z->thisseq->frst)
		...
		{	if (move_else)
			...
			ref_chans |= has_chanref(z->thisseq->frst->n);
		prev_z = z;
	}
	if (move_else)
	...
	if (prev_z
	&&  ref_chans
	&&  prev_z->thisseq->frst->n->ntyp == ELSE)
	{	prev_z->thisseq->frst->n->val = 1;
		has_badelse++;
	...
	for (z = s; z; prev_z = z, z = z->nxt)
		add_el(t, z->thisseq);	/* append target */
	if (tok == DO)
	{	add_el(t, cur_s->thisseq); /* target upfront */
		t = new_el(nn(n, BREAK, ZN, ZN)); /* break target */
		set_lab(break_dest(), t);	/* new exit  */
		breakstack = breakstack->nxt;	/* pop stack */
	}
	add_el(e, cur_s->thisseq);
	add_el(t, cur_s->thisseq);
...
{	SeqList *z;
	...
	f->esc = seqlist(e, f->esc);	/* lifo order... */
#ifdef DEBUG
	...
	switch (f->n->ntyp) {
	case UNLESS:
		attach_escape(f->sub->thisseq, e);
		break;
	case IF:
	case DO:
		for (z = f->sub; z; z = z->nxt)
			attach_escape(z->thisseq, e);
		break;
	case D_STEP:
		/* attach only to the guard stmnt */
		escape_el(f->n->sl->thisseq->frst, e);
		break;
	case ATOMIC:
	case NON_ATOMIC:
		/* attach only to all stmnts */
		attach_escape(f->n->sl->thisseq, e);
		break;
	}
}
...
{	SeqList	*s  = n->sl;
	...
	attach_escape(s->thisseq, s->nxt->thisseq);
	...
	{	Element *x; printf("\t%d,%d,%d :: ",
		z->thisseq->frst->Seqno,
		z->thisseq->extent->Seqno,
		z->thisseq->last->Seqno);
		for (x = z->thisseq->frst; x; x = x->nxt)
			printf("(%d)", x->Seqno);
		printf("\n");
	}
#endif
...
{	Element *f; Symbol *ofn; int oln;
	...
	{	f->status |= (ATOM|added);
		switch (f->n->ntyp) {
		case ATOMIC:
			printf("warning: line %3d %s, atomic inside %s (ignored)\n",
			  f->n->ln, f->n->fn->name, (added)?"d_step":"atomic");
			f->n->ntyp = NON_ATOMIC;
			goto mknonat;
		case D_STEP:
			printf("warning: line %3d %s, d_step inside ",
			 f->n->ln, f->n->fn->name);
			if (added)
		...
		case NON_ATOMIC:
mknonat:		h = f->n->sl;
			walk_atomic(h->thisseq->frst, h->thisseq->last, added);
			break;
		}
		for (h = f->sub; h; h = h->nxt)
			walk_atomic(h->thisseq->frst, h->thisseq->last, added);
