...
{	int oln = lineno;
	...
	{	lineno = n->ln;
		...
		{	if (strncmp(vis->sym->name, ":hide:", 6) == 0)
				n->sym->hidden |= 1;
			if (strncmp(vis->sym->name, ":show:", 6) == 0)
				n->sym->hidden |= 2;
		}
...
{	Symbol *tl;
	...
	return cast_val(tl->type, tl->val[ix]);
}
...
{	Symbol *tl;
	...
	strcat(buf, lbuf);
	if (tmp->sym->nel > 1)
	{	sprintf(lbuf, "[%d]", eval(tmp->lft,&ff));
		strcat(buf, lbuf);
	}
}
...
int
fill_struct(Symbol *z, int cur, int ov, char *gather, Symbol **sp, int how)
{	Lextok *fp, *tl;
	int ix;
	int nc = cur;
	int nv = ov;
	...
	{	if (tl->sym->type == STRUCT)
		{	if (z->nel > 1)
				sprintf(newpref, "%s[%d].%s",
					gather, ix, tl->sym->name);
			else
				sprintf(newpref, "%s.%s",
					gather, tl->sym->name);
			nc = fill_struct(tl->sym, nc, nv, newpref, sp, how);
			if (nc < 0)
			{	strcpy(gather, newpref);
				return nc;
			}
			if (nv < nc) nv = nc;
			continue;
		...
		if (tl->sym->type == CHAN
		&&  nv-nc < tl->sym->nel)
		{	strcpy(newpref, gather);
			if (z->nel > 1)
				sprintf(gather, "%s[%d]", newpref, ix);
			strcat(gather, ".");
			*sp = tl->sym;
			return -nv-1;
		}
