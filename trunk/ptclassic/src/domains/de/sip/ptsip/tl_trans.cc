...
Mapping	*Mapped = (Mapping *) 0;
Graph	*Nodes_Set = (Graph *) 0;
Graph	*Nodes_Stack = (Graph *) 0;
...
static int	Dump_cond(Node *, Node *, Node *, int, int);
...
static int
Dump_cond(Node *pp, Node *r, Node *r2, int full, int first)
{	Node *q;
	int frst = first;
	...
	{	if (!anywhere(AND, q, r2))
		...
		&& !anywhere(AND, q->rgt, r2))
	{	frst = Dump_cond(q->rgt, r, r2, full, frst);
	} else	if (q->ntyp == AND)
	{	frst = Dump_cond(q->lft, r, r2, full, frst);
		frst = Dump_cond(q->rgt, r, r2, full, frst);
	}
	return frst;
}
...
{	Node *q;
	...
	q = dupnode(pp);
	q = rewrite(q);
	return Dump_cond(q, r, r2, full, first);
}
...
static void
ng(Symbol *s, Symbol *in, Node *newnode, Node *old, Node *next)
{	Graph *g = (Graph *) tl_emalloc(sizeof(Graph));
	...
	if (newnode)  g->New  = flatten(newnode);
	if (old)  g->Old  = Duplicate(old);
