...
the SrcGrInt star with the named block at setup time.  The replacement
...
and output datatype to integer.  It also hides the input ports and the
\fIinput_map\fR parameter.
.Sr "MapGr"
	}
	constructor {
	  output.setPort("output", this, INT);
	  exin.setPort("exin", this, INT);
