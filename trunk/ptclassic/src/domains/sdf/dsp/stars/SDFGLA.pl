...
A number between 0 to 1. When equal to 0, the resulting codebook will satisfy the necessary conditions for optimality.
  ...
  defstate {
    ...
    type { int }
    default { 100 }
    desc { The upper limit on the iterations in the Lloyd algorithm. }
  }
  defstate {
    name { saveFile }
    type { string }
  ...
  code {
	...
	for (int k = 0; k < dimension; k++) {
	  distance += trnVector[k] * trnVector[k];
	}
     }	// end of NN function
  }	// end of code
  ...
  setup {
    ...
    //  Allocate an array to counter how many input training vectors
    //  are closest to the i_th codeword
    delete [] numPart;
  ...
  go {
    ...
    //  numPart[i] stores the number of how many input training vectors
    ...
    for (i = 0; i < int(sizeTrnSet); i++) {
      NN(index, distance, &(trnSet[i*int(dimension)]), codebook,
         halfCodewordPower, int(sizeCodebook), int(dimension));
      (numPart[index])++;
      distortion += distance;
      for (int j = 0; j < int(dimension); j++) {
    ...
    do {
      int i;
      //  Reset the improved codebook to the centroid of partitioned cluser set
      for (i = 0; i < int(sizeCodebook); i++) {
	int rowloc = i * int(dimension);
	//  Update ith code word
	//  Avoid division-by-zero error when numPart[i] is not zero
        if ( numPart[i] ) {
          for (int j = 0; j < int(dimension); j++) {
            codebook[rowloc + j] = centroid[rowloc + j] / numPart[i];
	  }
        }
      }
	...
	int rowloc = i * int(dimension);
      ...
      // for the codebook. Then find centroid of each partitioned cluster set.
      for (i = 0; i < int(sizeTrnSet); i++) {
	NN(index, distance, &(trnSet[i*int(dimension)]), codebook,
	   halfCodewordPower, int(sizeCodebook), int(dimension));
	(numPart[index])++;
