...
limitation of liability, and disclaimer of warranty provisions.
	...
	if ( strcasecmp( dt, "Err-Diffusion") == 0 ) {
	       ...
	       Error::abortRun(*this,
			       "The number of elements in 'ownMask' ",
			       "doesn't match the specified dimension.");
	       return;
            }
            mask = new double[rowsMask*colsMask];
            double *usermask = (double *) ownMask;
	    for (int i = 0; i < rowsMask*colsMask; i++ ) {
	      mask[i] = usermask[i];
	    }
	}
	else {
	    Error::abortRun(*this, "Unknown dither type ", dt);
	    return;
	}
    }
    go {
	  ...
	  if ( strcasecmp( dt, "Err-Diffusion")==0 ) {
	    ...
	    // Do clustered dither or dispersed dither depending on mask
	    for (int i = 0; i < height; i++) {
	      for (int j = 0; j < width; j++) {
