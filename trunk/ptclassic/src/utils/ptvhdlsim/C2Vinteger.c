     ...
     cliIID iid;
{
  ...
  (void) sprintf(idata->nearstring, "/tmp/rcv%d\0", idata->pairid);
  (void) sprintf(idata->farstring, "/tmp/snd%d\0", idata->pairid);
...
/********************
   Evaluation Routine
********************/
void c2vinteger_eval(did,iid,ppid,np)
     cliDID did;
     cliIID iid;
     cliPID *ppid;
     int np;
...
/*
      perror("c2vinteger Connection Established");
*/
  ...
  (void) sscanf(idata->buffer, idata->format, &(idata->intptr));

      svData.value.integer = idata->intptr;
      svData.typemark = CLI_TYPE_INTEGER;
      svData.length = 0;
...
************************/
void c2vinteger_error(did,iid,level)
     cliDID did;
     cliIID iid;
     int level;
{
